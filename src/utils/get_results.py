...
def highlight_top_wines_1():
    """
    Q1: Highlights 10 wines to increase sales based on specific criteria such as sales trends,
    ratings, and uniqueness. 
    Approach here is by a measure of: popular wines, of good quality and many harvests.
    """
...
def highlight_top_wines_2():
    ...
    Q1: Highlights 10 wines to increase sales based on specific criteria such as sales trends,
    ratings, and uniqueness. 
    Approach here is getting 5 popular types of wine and showing top 10 popular by reviews on those types.
    """
    df_results = select_query_to_pandas("""
    WITH top_types AS (
        SELECT 
            fv.fk_wine_id AS wine_id, 
            SUM(fv.ratings_count) as total_reviews, 
            dt.toplists_type_wine as type_wine
        FROM Fact_vintages fv 
        JOIN Dim_toplists dt 
            ON dt.toplists_id = fv.fk_last_toplist
        GROUP BY fv.fk_wine_id
        ORDER BY total_reviews DESC
    )
    
    SELECT top_types.wine_id, dw.wine_name, type_wine, spw.average_weighted_price, spw.numb_vintages, spw.total_count , spw.total_sale  
    FROM top_types
    JOIN Dim_wines dw 
        ON dw.wine_id = top_types.wine_id
    JOIN sales_per_wine spw 
        ON spw.name = dw.wine_name
    WHERE type_wine 
        IN (SELECT DISTINCT type_wine
            FROM top_types
            LIMIT 5) 
    AND numb_vintages > 5
